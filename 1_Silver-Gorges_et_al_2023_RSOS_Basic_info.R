...
scute_factors<-c("FlipperTag1", 
                 "FlipperTag2", 
                 "PITTag", 
                 "Sex", 
                 "LifeStage",
                 "TurtleID", 
                 "Depthum") #select appropriate vars to convert into factors for scute data
...
##Just In Case: Fix Life Stage to Subadult<80<Adult
...
scute_working<- scute %>%
  ...
  drop_na(d13C) #removes NAs

skin_working<- skin %>%
  drop_na(d15NEpi) %>% #removes NAs
...
##Plots
#skin
jpeg(filename="Figure2.jpeg", width=8, height=8, units="in", res=350)
skin_working %>%
...
#Create Figure 3#
#Note that d13C axis will not appear negative in order to allow plots to line up. "-" added later!
jpeg("Figure3.jpg", width = 10, height = 10, units = "in", res = 350) 
